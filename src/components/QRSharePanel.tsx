...
import { QrCode, Wifi, WifiOff, Users, ChevronDown, ChevronUp, Copy, Check } from "lucide-react";
    ...
    <GlassCard glow="purple" className="flex flex-col gap-3">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-200">Share to Devices</p>
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full ${
            connected ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-500/10 text-slate-500"
          }`}>
            {connected
            ...
            <div className="flex flex-col gap-3 pt-1">
              {viewerUrls.length > 1 && (
                <div className="relative">
                  ...
                  >
                    {viewerUrls.map((u) => (
                      <option key={u} value={u} className="bg-[#111114]">{u}</option>
                    ))}
                  </select>
              ...
              {qrDataUrl && (
                <div className="flex justify-center">
                  <div className="rounded-2xl overflow-hidden border border-white/10 p-2 bg-[#0f0f1a]">
                    <img src={qrDataUrl} alt="Viewer QR code" className="w-40 h-40" />
                  </div>
                </div>
              )}

              <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-2 border border-white/10">
                <p className="text-[10px] text-slate-400 flex-1 truncate font-mono">{selectedUrl}</p>
                <button
                  onClick={copyUrl}
              ...
              <p className="text-[10px] text-slate-600 text-center">
                Other devices on the same Wi-Fi can scan this to see live translations
              </p>
            </div>
          </motion.div>
        )}
